...
#define MIN_SCORE (-10000000)
	...
	int evaluatePoint(char board[BOARD_WIDTH][BOARD_WIDTH], Position p) {
		...
		string lines[4];
		string lines1[4];
		for (i = max(0, p.x - 5); i < (int)min(BOARD_WIDTH, p.x + 6); i++) {
			if (i != p.x) {
		...
		for (i = max(0, p.y - 5); i < (int)min(BOARD_WIDTH, p.y + 6); i++) {
			if (i != p.y) {
		...
		for (i = p.x - min(min(p.x, p.y), 5), j = p.y - min(min(p.x, p.y), 5); i < (int)min(BOARD_WIDTH, p.x + 6) && j < (int)min(BOARD_WIDTH, p.y + 6); i++, j++) {
			if (i != p.x) {
		...
		for (i = p.x + min(min(p.y, BOARD_WIDTH - 1 - p.x), 5), j = p.y - min(min(p.y, BOARD_WIDTH - 1 - p.x), 5); i >= (int)max(0, p.x - 5) && j < (int)min(BOARD_WIDTH, p.y + 6); i--, j++) {
			if (i != p.x) {
